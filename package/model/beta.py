...
from package.utils import dense
...
class BetaModel(torch.nn.Module):
    ...
    def __init__(self, config: BetaConfig):
        ...
        self.pre_crf = ConditionalRandomField(self.pre_tag_size)
        ...
        self.arg_crf = ConditionalRandomField(self.arg_tag_size)

    def state_dict(self):
        """
        Exclude the weights of LMs
        """
        state_dict = super(BetaModel, self).state_dict()
    ...
    def forward(self, text: Union[str, List[str]]) -> List[Dict]:
        ...
        pre_tags = self.pre_crf(hidden_pre, attention_mask)
        ...
        for i in range(B):
            ...
            if P:
                ...
                arg_tags = self.arg_crf(hidden_arg)
    ...
    def loss(self, input_ids, attention_mask, pre_label_all, pre_label, arg_label):
        ...
        hidden_pre = self._predict_predicate(hidden)
        loss_pre = self.pre_crf.neg_log_likelihood_loss(hidden_pre, attention_mask, pre_label_all)
        ...
        loss_arg = self.arg_crf.neg_log_likelihood_loss(hidden_arg, attention_mask, arg_label)
